...
export default define({
  ...
  render: {
    value: ({ items, selected, cropTarget, existingTags, publishing }) => {
      const pending = Array.isArray(items) ? items.filter((i) => i.status !== 'complete') : [];
      ...
      return html`
        ...
        <main class="media-drafts-page">
          ...
          <div
            ...
            ondrop="${(host, e) => {
              ...
              const files = [...e.dataTransfer.files].filter((f) =>
                /\.(jpe?g|png|gif|webp|svg|mp4|webm|mov|avi|nef|cr2|arw|dng|raf|rw2)$/i.test(
                  f.name,
                ),
              );
          ...
          ${selected.length >= 2 && pending.some((i) => selected.includes(i.slug))
            ? html`
                <div class="media-drafts__actions">
                  ...
                  >
                    Merge ${selected.length} into carousel
                  </button>
                  <button
                    class="btn btn-small"
                    onclick="${(host) => {
          ...
          ${pending.length === 0 && items !== undefined
            ? html`<p class="media-drafts__empty">No media drafts. Drop files above to start.</p>`
            : ''}
          ...
          <div class="media-drafts__grid">
            ${pending.map(
              (item) => html`
                <div
                  class="${{
                    'media-drafts__card': true,
                    selected: selected.includes(item.slug),
                    publishing: publishing[item.slug] === 'publishing',
                    published: publishing[item.slug] === 'published',
                  }}"
                  onclick="${(host, e) => {
                ...
                >
                  <div class="media-drafts__thumb">
                    ...
                    ${isVideo(item.files[0]) && item.proxy
                      ? html`<img src="${thumbSrc(item)}" alt="" /><span class="media-drafts__play"
                            >▶</span
                          >`
                      : isVideo(item.files[0])
                        ? html`<video
                            src="${API}/file/${encodeURIComponent(item.files[0])}"
                            muted
                            preload="metadata"
                          ></video>`
...
${item.caption}</textarea
                    ...
                    <div class="media-drafts__tag-suggest">
                      ${suggestTags(item.caption, item.tags).map(
                        (t) => html`
                          <button
                            class="tag-pill tag-pill--suggest"
                            onclick="${(host) => {
                      ...
                      ${existingTags
                        ...
                        .map(
                          (t) => html`
                            <button
                              class="tag-pill tag-pill--existing"
                              onclick="${(host) => {
                                const tags = [...item.tags, t];
                                patchItem(item.slug, { tags });
                                item.tags = tags;
                    ...
                    <div class="media-drafts__card-actions">
                      ...
                      >
                        ${isVideo(item.files[0]) ? 'Crop' : 'Edit'}
                      </button>
                      <button
                        class="btn btn-small btn-primary"
                        disabled="${!!publishing[item.slug]}"
                        onclick="${(host) => {
                          publishWithAnimation(host, item.slug);
                        }}"
                      >
                        ${publishing[item.slug] === 'publishing'
                          ? 'Publishing…'
                          : publishing[item.slug] === 'published'
                            ? '✓'
                            : 'Publish'}
                      </button>
                      <button
                  ...
                  <div class="crop-modal__inner">
                    ...
                    <details open>
                      ...
                      <div class="crop-modal__ratios">
                        ${['1:1', '4:5', '16:9', '3:2'].map(
                          (r) => html`
                            <button
                              class="${{
                                'btn btn-small': true,
                                'btn-primary': cropTarget.crop?.ratio === r,
                              }}"
                              onclick="${(host) => {
                      ...
                      ${cropTarget.crop
                        ? html`<p class="crop-modal__hint">Click image to set focal point</p>`
                        : ''}
                    </details>

                    ${!isVideo(cropTarget.files[0])
                      ? html`
                          <details>
                              ...
                              ${[
                                ...
                                ([name, min, max, step]) => html`
                                  <label class="crop-modal__slider">
                                    ...
                                    />
                                    <span class="crop-modal__slider-val"
                                      >${cropTarget.filters?.[name] || 0}</span
                                    >
                                  </label>
                                `,
                              )}
