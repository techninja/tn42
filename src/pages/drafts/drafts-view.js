...
export default define({
  ...
  drafts: {
    value: undefined,
    connect(host) {
      loadDrafts().then((d) => { host.drafts = d; });
      setPageTitle('Drafts');
    },
  },
  publishing: { value: {} },
  ...
  render: {
    value: ({ drafts, filter, hideComplete, publishing }) => {
      ...
      const counts = {};
      if (Array.isArray(drafts)) drafts.forEach((d) => { counts[d.status] = (counts[d.status] || 0) + 1; });
      ...
      return html`
        ...
        <main class="drafts-page">
          ...
          ${drafts === undefined ? html`<p>Loading…</p>` : html`
            <div class="drafts-toolbar">
              ...
              <label class="drafts-toolbar__toggle">
                <input
                  ...
                  onchange="${(host, e) => { host.hideComplete = e.target.checked; }}"
                />
                Hide complete
              </label>
            </div>
              ...
              <tbody>
                ${visible.map((d) => html`
                  <tr>
                    ...
                    <td><a href="${router.url(DraftDetailView, { slug: d.slug })}">${d.title}</a></td>
                    <td>${d.images.length}</td>
                    <td>
                      ${d.status === 'ready' ? html`
                        <button
                          class="btn btn-small btn-primary btn-publish ${publishing[d.slug] || ''}"
                          disabled="${!!publishing[d.slug]}"
                          onclick="${(host) => {
                            host.publishing = { ...host.publishing, [d.slug]: 'publishing' };
                            publishDraft(d.slug).then((result) => {
                              if (result.success) {
                                host.publishing = { ...host.publishing, [d.slug]: 'published' };
                                setTimeout(() => loadDrafts().then((all) => { host.drafts = all; }), 1200);
                              } else {
                        ...
                        >${publishing[d.slug] === 'publishing' ? 'Publishing…' : publishing[d.slug] === 'published' ? '✓ Published' : 'Publish'}</button>
                      ` : ''}
                    </td>
                  </tr>
                `)}
            ...
            ${visible.length === 0 ? html`<p class="drafts-page__empty">No drafts matching filter.</p>` : ''}
          `}
        </main>
