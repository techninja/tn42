...
export default define({
  ...
  draft: {
    value: undefined,
    connect(host) {
      if (host.slug) loadDraft(host.slug).then((d) => {
        host.draft = d || false;
  ...
  render: {
    value: ({ draft, slug, saving, processResult }) => html`
      ...
      <main class="draft-editor">
        <app-breadcrumb items="${JSON.stringify([
          { label: 'Home', href: '/' },
        ...
        ${draft && draft.title ? html`
          <div class="draft-editor__layout">
            <section class="draft-editor__meta">
                  ...
                  onchange="${(host, e) => {
                    ...
                    if (newSlug && newSlug !== slug) {
                      ...
                      fetch(`/_api/drafts/${slug}/rename`, {
                        ...
                        else { alert(result.error || 'Rename failed'); host.saving = false; }
                      });
                    }
                  }}"
                />
              ...
              <label class="draft-editor__label">
                ...
                >
                  ${STATUS_OPTIONS.map((s) => html`
                    <option value="${s}" selected="${draft.status === s}">
                      ${s}
                    </option>
                  `)}
                </select>
              ...
              >⚙ Process Assets for Web</button>
              ${processResult ? html`
                <p class="draft-editor__process-result">
                  ✓ ${processResult.processed?.length || 0} files processed
                  ${processResult.errors?.length ? html`<br/>⚠ ${processResult.errors.join(', ')}` : ''}
                </p>
              ` : ''}

              <h3>Images (${draft.images.length})</h3>
              <div class="draft-editor__images">
                ${draft.images.map((img, i) => html`
                  <div class="draft-editor__img-card">
                    ${/\.(mp4|webm|mov)$/i.test(img.file)
                      ? html`<video src="/_api/drafts/${slug}/images/${img.file}" muted preload="metadata"></video>`
                      : html`<img src="/_api/drafts/${slug}/images/${img.file}" alt="${img.caption}" />`}
                    <input
                      type="text"
                      placeholder="Caption…"
                      value="${img.caption}"
              ...
              <div
                ...
                ondrop="${(host, e) => {
                  ...
                  const files = [...e.dataTransfer.files].filter((f) => /\.(jpe?g|png|gif|webp|svg|mp4|webm|mov|avi)$/i.test(f.name));
                  ...
                  fetch(`/_api/drafts/${slug}/images`, { method: 'POST', body: form })
                    ...
                    .then((result) => {
                      ...
                      const images = [...host.draft.images, ...uploaded.map((f) => ({ file: f, caption: '' }))];
                      return saveDraft(slug, { images });
                    })
                    .then((d) => { host.draft = { ...host.draft, ...d, availableImages: [...(host.draft.availableImages || []), ...files.map((f) => f.name)] }; host.saving = false; });
                }}"
              >
                <p>Drop images here</p>
              </div>

              ${draft.availableImages?.length ? html`
                    ...
                    .map((f) => html`
                      ...
                      >
                        ${/\.(mp4|webm|mov)$/i.test(f)
                          ? html`<video src="/_api/drafts/${slug}/images/${f}" muted preload="metadata"></video>`
                          : html`<img src="/_api/drafts/${slug}/images/${f}" alt="${f}" />`}
                        <span>+ ${f}</span>
                      </button>
                    `)}
                </div>
