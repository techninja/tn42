...
const stopanim = false;
const oneshotperscreen = false; // Just like it says!
const shipscreenwrap = true; // Wrap the ships on the screen
  ...
  $.each(ships, function () {
    if (this.exploding) {
      $('#boom_' + this.element_name).css({
        left: this.pos.x - this.width / 2 - 64,
        top: this.pos.y + this.height / 2 - 128,
      });
    }
...
function update_projectile_movement() {
  for (let s = 0; s < ships.length; s++) {
    for (let p = 0; p < ships[s].projectiles.length; p++) {
    ...
    for (let p = 0; p < ships[s].projectiles.length; p++) {
      ...
      if (proj.active) {
        ...
        proj.element
          ...
          .removeClass('init');
      } else if (proj.element.length) {
        // Cull inactive projectile elements
        proj.element.remove();
      }

      if (
        proj.pos.x > $(window).width() ||
...
function update_ship_movement() {
  $.each(ships, function () {
    ...
    this.velocity_length =
      Math.sqrt(Math.pow(this.velocity_x, 2) + Math.pow(this.velocity_y, 2)) - this.drag;
    ...
    if (this.velocity_length < 0) {
      ...
      const rot = Math.atan2(this.velocity_y, this.velocity_x) * (180 / Math.PI);
    ...
    if ((this.velocity_length != 0 || this.turn != 0) && shipscreenwrap) {
      if (this.pos.y < 0) {
        // Create Clone
        if ($(this.element_id).length == 1) this.element.clone().appendTo('body');

        // Move to Bottom if past top
        if (-this.pos.y >= 64) this.pos.y = $(window).height() - 64;

        // Position Clone
        $(this.element_id + ':last').css({
          left: this.pos.x + 'px',
          top: $(window).height() + this.pos.y + 'px',
        });
      }
      ...
      if (this.pos.x < 0) {
        ...
        if (this.pos.x <= -64) this.pos.x = $(window).width() - 64;

        // Position Clone
        $(this.element_id + ':last').css({
          left: $(window).width() + this.pos.x + 'px',
          top: this.pos.y + 'px',
        });
      }
      ...
      if (this.pos.y + 64 > $(window).height()) {
        ...
        if (this.pos.y > $(window).height()) this.pos.y = 0;
      ...
      if (this.pos.x + 62 >= $(window).width()) {
        ...
        if (this.pos.x >= $(window).width()) this.pos.x = 0;

        // Position Clone
        $(this.element_id + ':last').css({
          left: this.pos.x - $(window).width() + 'px',
          top: this.pos.y + 'px',
        });
      }
      //*/
      ...
      if (
        ...
        this.pos.y + 64 < $(window).height() &&
        $(this.element_id).length > 1
      ) {
